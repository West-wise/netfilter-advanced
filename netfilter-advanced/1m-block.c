...
/* returns packet id */
static u_int32_t print_pkt (struct nfq_data *tb, TrieNode *root)
{
	int id = 0;
		...
		if (ip_header->protocol == IPPROTO_TCP) {
			...
			if (host_start) {
				...
				if (host_end) {
					...
					// 호스트 주소를 문자열로 추출
					char host[100]; // 적절한 크기로 변경
					strncpy(host, (char *)host_start, host_length);
					host[host_length] = '\0';

					printf("Host: %s\n",host);
					if(search(root,host)==1){
						print_warning();
						return -1;
					}
					else{
...
static int cb(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg, struct nfq_data *nfa, void *data) {
	...
	//KeyValueEntry *hashMap = (KeyValueEntry *)data
	TrieNode *trie = (TrieNode *)data;
    u_int32_t id = print_pkt(nfa,trie);
    if (id == (u_int32_t)-1) {
...
int main(int argc, char **argv)
{
    ...
    while (fgets(line, sizeof(line), file)) {
        char *number = strtok_r(line, ",", &saveptr);
	...
	*/
	...
	qh = nfq_create_queue(h,  0, &cb, root);
	if (!qh) {
	...
	*/
	...
	//시그널 변수(?)를 통해서 iptable -F명령어가 제대로 인식되도록 하자!
	free_iptable();
